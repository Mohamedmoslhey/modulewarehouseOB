...
public class CusSRMOPickEditLines extends BaseProcessActionHandler {
  ...
  private void createOrderLines(JSONObject jsonRequest, List<String> idList)
    ...
    List<OrderLine> linesList = order.getOrderLineList();
    ...
    for (long i = 0; i < selectedLines.length(); i++) {
      ...
      newOrderLine.setPAPERJumboCode(selectedLine.getString("paperJumboCode"));
      newOrderLine.setPAPERBakaraWeight(new BigDecimal(selectedLine.getString("paperBakawieght")));
      newOrderLine.setPAPERBakaraWidth(new BigDecimal(selectedLine.getString("paperBakarawidth")));
      newOrderLine.setPAPERBakkaraQotr(new BigDecimal(selectedLine.getString("paperBakaraqotr")));
      newOrderLine.setPAPERGsm(new BigDecimal(selectedLine.getString("paperGsm")).longValue());
      newOrderLine.setPAPERPaperClient(OBDal.getInstance().get(BusinessPartner.class,
          selectedLine.getString("pAPERPaperClient")));
      newOrderLine.setPAPERPaperMachine(
          OBDal.getInstance().get(PaperMachine.class, selectedLine.getString("paperMachine")));
      newOrderLine.setPAPERProductType(selectedLine.getString("paperProductType"));
      newOrderLine.setPAPERQualityDegree(OBDal.getInstance().get(QualityDegree.class,
          selectedLine.getString("paperQualityDegree")));
      if (!selectedLine.getString("paperRejectreason").equals("null")) {
        newOrderLine.setPaperRejectreason(selectedLine.getString("paperRejectreason"));
      } else {
        newOrderLine.setPaperRejectreason("");
      }
      if (!selectedLine.getString("paperNotes").equals("null")) {
        newOrderLine.setPaperNotes(selectedLine.getString("paperNotes"));
      } else {
        newOrderLine.setPaperNotes("");
      }
    ...
    order.setPAPERNumberOfRoller(0L);
    order.setPAPERTotalShipmentWeight(new BigDecimal("0"));

    for (long i = 0; i < selectedLines.length(); i++) {

      totalItems++;
      order.setPAPERTotalShipmentWeight(order.getPAPERTotalShipmentWeight()
          .add(new BigDecimal(selectedLines.getJSONObject((int) i).getString("paperBakawieght"))));
      order.setPAPERNumberOfRoller(totalItems);
    }
    OBDal.getInstance().save(order);
    OBDal.getInstance().flush();
